...
class APIWrapper__edit_blacklist_monitorTest ( unittest.TestCase ):
    def setUp ( self ) -> None:
        ...
        self.expected_object_simple.json = Mock ( return_value = {
            "status": "SUCCESS",
            "message": "monitor has been edited"
        } );
    ...
    def test_edit_blacklist_monitor_success ( self ):
        """API Instance"""
        with patch ( 'requests.post' ) as patch_request_post:
            ...
            returned_object = api_wrapper.edit_blacklist_monitor (
                *self.dummy_param_simple
            );
        self.assertEqual ( self.expected_object_simple, returned_object );

    def test_edit_blacklist_monitor_connection_error ( self ):
    ...
    def test_edit_blacklist_monitor_connect_timeout ( self ):
        ## Simulate connection problem
        import socket, requests;
        def guard ( *args, **kwargs ):
